...
print(f"ðŸ”§ Final Fix for Virtual Space UI (Clickable & Stable): {TARGET_FILE}")

# --- FULL CORRECTED CODE ---
code_content = r"""import React, { useState, useCallback } from "react";
import SidebarModern from "../components/SidebarModern";
import { Video, Mic, MicOff, Monitor, Share2, Youtube, FileText, PhoneOff, VideoOff, Radio, UploadCloud, Cast, Layers, Settings, Phone } from "lucide-react";
  ...
  return (
    <div className="virtual-space-container">
            ...
            <div className="vs-grid-layout fade-in-up">
                ...
                <div className="video-stage-card glass-panel">
                    <div className="video-container">
                        ...
                        {/* Status Overlays - Fixed Z-Index */}
                        <div className="status-overlays" style={{pointerEvents: 'none'}}>
                        ...
                        {/* Floating Control Bar - High Z-Index & Clickable */}
      ...
      <style>{`
        :root {
            --vs-bg: #09090b;
            --vs-card-bg: rgba(255, 255, 255, 0.03);
        ...
        .hover-scale { transition: transform 0.2s cubic-bezier(0.34, 1.56, 0.64, 1); cursor: pointer; }
        .hover-scale:active { transform: scale(0.95) !important; }
        .hover-scale:hover { transform: scale(1.05); }
        .hover-lift { transition: transform 0.2s, background 0.2s; cursor: pointer; }
        .hover-lift:hover { transform: translateY(-4px); }
        .hover-slide { transition: transform 0.2s, background 0.2s, border-color 0.2s; cursor: pointer; }
        .hover-slide:hover { transform: translateX(8px); }
        .hover-glow { transition: box-shadow 0.3s, border-color 0.3s; }
        .hover-glow:hover { border-color: rgba(139, 92, 246, 0.5); box-shadow: 0 0 20px rgba(139, 92, 246, 0.2); }
        ...
        .btn-luxe { border: none; padding: 12px 24px; border-radius: 14px; color: white; font-weight: 700; display: flex; align-items: center; gap: 10px; font-size: 0.95rem; box-shadow: 0 4px 15px rgba(0,0,0,0.2); transition: all 0.3s; }
        ...
        .control-bar-floating { position: absolute; bottom: 30px; left: 50%; transform: translateX(-50%); display: flex; gap: 12px; padding: 12px 20px; border-radius: 60px; z-index: 50; align-items: center; cursor: default; }
        .glass-control { background: rgba(24, 24, 27, 0.85); backdrop-filter: blur(20px); -webkit-backdrop-filter: blur(20px); border: 1px solid rgba(255, 255, 255, 0.15); box-shadow: 0 8px 32px rgba(0,0,0,0.5); }
        
        .control-btn-luxe { width: 54px; height: 54px; border-radius: 50%; border: 1px solid rgba(255,255,255,0.1); background: rgba(255,255,255,0.05); color: var(--vs-text-primary); display: flex; alignItems: center; justifyContent: center; cursor: pointer; transition: all 0.2s; position: relative; z-index: 60; }
        .control-btn-luxe:hover { background: rgba(255,255,255,0.2); border-color: rgba(255,255,255,0.4); transform: scale(1.1); }
        ...
        .btn-hangup:hover { background: #dc2626 !important; box-shadow: 0 8px 25px rgba(239, 68, 68, 0.5); transform: scale(1.1); }
        ...
        .panel-card { display: flex; flex-direction: column; height: 100%; max-height: 48%; }
        ...
        .broadcast-item.youtube { color: #ff0000; }
        .broadcast-item.zoom { color: #2d8cff; }
        .broadcast-item.meet { color: #00ac47; }
...
print("âœ… SUCCESS: VirtualSpace.jsx Fixed (Audio Echo Removed & Buttons Clickable)!")
