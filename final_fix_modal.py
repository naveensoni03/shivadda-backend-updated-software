...
print(f"🚀 Adding Detail Modal & Pagination Visibility in: {TARGET_FILE}")

code_content = r"""import React, { useState, useEffect } from "react";
import SidebarModern from "../components/SidebarModern";
import { Globe, Plus, RefreshCw, MoreHorizontal, ChevronLeft, ChevronRight, Zap, Eye, Edit, Trash2, X, MapPin, Mail } from "lucide-react";
...
export default function Locations() {
  ...
  const [viewNode, setViewNode] = useState(null); // Modal State
  ...
  return (
    <div className="waoo-app-container">
                            ...
                            {currentItems.map((center, i) => (
                                <tr key={center.id} className="waoo-row">
                                    ...
                                    <td style={{textAlign: 'center', position: 'relative'}}>
                                        <button className="row-action-btn" onClick={() => setActiveActionMenu(activeActionMenu === center.id ? null : center.id)}>
                                            <MoreHorizontal size={20}/>
                                        </button>
                                        
                                        {activeActionMenu === center.id && (
                                            <div className="action-popup fade-in">
                                                <button onClick={() => { setViewNode(center); setActiveActionMenu(null); }}><Eye size={14}/> View Node</button>
                                                <button onClick={() => { toast("Edit Mode: " + center.name); setActiveActionMenu(null); }}><Edit size={14}/> Edit</button>
                                                <button className="delete" onClick={() => { toast.error("Delete: " + center.name); setActiveActionMenu(null); }}><Trash2 size={14}/> Remove</button>
                                            </div>
                                        )}
                                    </td>
                                </tr>
                ...
                <div className="modal-overlay fade-in">
                    <div className="modal-content scale-up-bounce">
                        ...
                        <div className="modal-footer">
                            <button className="btn-close" onClick={() => setViewNode(null)}>Close Terminal</button>
                        </div>
                    </div>
                </div>
            )}
        ...
        <style>{`
            ...
            .premium-select { padding: 14px; border-radius: 14px; border: 2px solid #f1f5f9; background: white; color: #0f172a; font-weight: 700; cursor: pointer; }
            ...
            .row-action-btn { background: #f1f5f9; border: none; width: 36px; height: 36px; border-radius: 10px; cursor: pointer; display: flex; align-items: center; justify-content: center; margin: 0 auto; }
            .action-popup { position: absolute; right: 50px; top: 0; background: white; border-radius: 12px; box-shadow: 0 10px 30px rgba(0,0,0,0.1); border: 1px solid #e2e8f0; padding: 8px; z-index: 100; display: flex; flex-direction: column; gap: 4px; min-width: 150px; }
            .action-popup button { background: transparent; border: none; text-align: left; padding: 10px 12px; border-radius: 8px; font-weight: 700; color: #475569; cursor: pointer; display: flex; align-items: center; gap: 8px; font-size: 0.85rem; }
            .action-popup button:hover { background: #f1f5f9; color: #3b82f6; }
            .action-popup button.delete:hover { background: #fee2e2; color: #ef4444; }
            ...
            /* --- PAGINATION STYLING --- */
            .waoo-pagination-bar { padding: 20px 30px; border-top: 1px solid #f1f5f9; background: #fcfcfc; }
            ...
            .current-p { font-weight: 800; color: #64748b; }
            ...
            .info-item label { display: block; font-size: 0.75rem; font-weight: 800; color: #94a3b8; text-transform: uppercase; margin-bottom: 4px; }
            .info-item p { margin: 0; font-weight: 700; color: #1e293b; line-height: 1.4; }

            .modal-footer { padding: 20px 30px; background: #fcfcfc; text-align: right; border-top: 1px solid #f1f5f9; }
...
print("✅ SUCCESS: Modal added and Pagination bar forced to show!")
