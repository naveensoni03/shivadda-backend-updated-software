...
print(f"ðŸ”§ Adding View Button & Pagination to Institutions Page in: {TARGET_FILE}")

code_content = r"""import React, { useState, useEffect } from "react";
import SidebarModern from "../components/SidebarModern";
import { Building2, Plus, Search, MapPin, User, Mail, MoreHorizontal, Edit, Trash2, X, Save, CheckCircle, Phone, Eye, ChevronLeft, ChevronRight } from "lucide-react";
...
export default function Institutions() {
  ...
  const [viewNode, setViewNode] = useState(null); // NEW: View State
    ...
    <div className="waoo-app-container">
        ...
        <div className="waoo-view">
            ...
            <div className="search-bar-container">
                ...
                <input type="text" placeholder="Search schools..." value={searchTerm} onChange={(e) => { setSearchTerm(e.target.value); setCurrentPage(1); }} className="search-input"/>
            </div>
            ...
            <div className="grid-container">
                            ...
                            <div className="card-actions">
                                ...
                                {activeMenu === inst.id && (
                                    <div className="menu-dropdown">
                                        <button onClick={() => { setViewNode(inst); setActiveMenu(null); }}><Eye size={14}/> View</button>
                ...
                {currentItems.length === 0 && !loading && <div className="empty-state">No institutions found.</div>}
            </div>
                ...
                <div className="modal-overlay fade-in">
                    <div className="modal-content scale-up">
                        <div className="modal-head">
                            ...
                            <button className="close-btn" onClick={() => setViewNode(null)}><X size={24}/></button>
                        </div>
                        <div className="modal-body view-mode">
                            <div className="view-row">
                                <span className="label">Name:</span>
                                <span className="value highlight">{viewNode.name}</span>
                            </div>
                            <div className="view-row">
                                <span className="label">Principal:</span>
                                <span className="value">{viewNode.principal_name || "N/A"}</span>
                            </div>
                            <div className="view-row">
                                <span className="label">Phone:</span>
                                <span className="value">{viewNode.phone || "N/A"}</span>
                            </div>
                            <div className="view-row">
                                <span className="label">Email:</span>
                                <span className="value">{viewNode.contact_email || "N/A"}</span>
                            </div>
                            <div className="view-row">
                                <span className="label">Address:</span>
                                <span className="value">{viewNode.address || "N/A"}</span>
                            </div>
                        </div>
                        <div className="modal-foot">
                            <button className="btn-primary" onClick={() => setViewNode(null)}>Close</button>
                        </div>
                    </div>
                </div>
            )}
        ...
        <style>{`
            ...
            .p-btn { background: white; border: 1px solid #e2e8f0; padding: 10px; border-radius: 10px; cursor: pointer; display: flex; align-items: center; justify-content: center; color: #0f172a; transition: 0.2s; }
            .p-btn:hover:not(:disabled) { background: #3b82f6; color: white; border-color: #3b82f6; }
            .p-btn:disabled { opacity: 0.5; cursor: not-allowed; }
            .p-info { font-weight: 700; color: #64748b; }

            /* VIEW MODE STYLES */
            .view-row { display: flex; justify-content: space-between; padding: 12px 0; border-bottom: 1px solid #f1f5f9; }
            .view-row:last-child { border-bottom: none; }
            .view-row .label { color: #64748b; font-weight: 600; }
            .view-row .value { color: #0f172a; font-weight: 700; text-align: right; }
            .view-row .highlight { color: #3b82f6; font-size: 1.1rem; }
...
print("✅ INSTITUTIONS UPDATE: View Button & Pagination Added!")
