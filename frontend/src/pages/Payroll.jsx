...
export default function Payroll() {
  ...
  // 🖨️ FUNCTION: Print Receipt (Simple window.print)
  ...
  // 🔥 NEW FUNCTION: Generate Professional PDF Invoice with jsPDF
  const handleDownloadReceipt = () => {
    ...
    doc.text("Education Hub, Vidyadhar Nagar", 14, 35);
    doc.text("Jaipur, Rajasthan, 302039", 14, 40);

    // Invoice Title
    doc.setFontSize(16);
    doc.setTextColor(15, 23, 42);
    doc.setFont("helvetica", "bold");
    doc.text("SALARY RECEIPT", 130, 22);

    doc.setFontSize(10);
    doc.setFont("helvetica", "normal");
    doc.text(`Receipt No: TXN-${selectedReceipt.id}8829`, 130, 30);
    doc.text(`Date: ${new Date(selectedReceipt.payment_date).toLocaleDateString()}`, 130, 35);
    ...
    doc.setFontSize(10);
    doc.setFont("helvetica", "normal");
    doc.text(`Name: ${selectedReceipt.staff_name}`, 14, 63);
    doc.text(`Role: ${selectedReceipt.role}`, 14, 69);
    doc.text(`Period: ${selectedReceipt.month}`, 14, 75);

    // Table Data
    const amountNum = Number(selectedReceipt.amount);

    doc.autoTable({
      startY: 85,
      head: [['Description', 'Earnings (INR)', 'Deductions (INR)', 'Net Pay (INR)']],
      body: [
        [`Basic Salary - ${selectedReceipt.month}`, `Rs. ${amountNum.toLocaleString()}`, `Rs. 0`, `Rs. ${amountNum.toLocaleString()}`]
      ],
      theme: 'grid',
    ...
    doc.setFontSize(12);
    doc.setFont("helvetica", "bold");
    doc.text(`Total Amount Transferred: Rs. ${amountNum.toLocaleString()}/-`, 14, finalY + 15);
    ...
    // Save
    doc.save(`Salary_Slip_${selectedReceipt.staff_name}_${selectedReceipt.month}.pdf`);
  ...
  return (
    <div className="payroll-page-wrapper">
      ...
      <div className="payroll-main-content">
        ...
        {/* 🌟 MODAL (Fully Functional PDF Generation) 🌟 */}
        <AnimatePresence>
          {selectedReceipt && (
              ...
              <motion.div initial={{ opacity: 0, scale: 0.9, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9 }} style={{ background: 'white', width: '100%', maxWidth: '400px', borderRadius: '24px', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.25)', overflow: 'hidden', margin: '20px', maxHeight: '90vh', display: 'flex', flexDirection: 'column' }}>
                ...
                <div style={{ padding: '25px', overflowY: 'auto', flex: 1 }}>
                  ...
                  <div style={{ display: 'flex', gap: '10px' }}>
                    ...
                    <button onClick={handleDownloadReceipt} style={{ flex: 1, padding: '12px', borderRadius: '12px', border: 'none', background: '#4f46e5', color: 'white', fontWeight: '700', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}><FileText size={16} /> Download PDF</button>
                  </div>
                </div>
              </motion.div>
            </div>
