...
export default function Library() {
  ...
  const inputStyle = { width: '100%', padding: '12px', borderRadius: '12px', border: '1px solid #334155', background: '#1e293b', color: '#ffffff', outline: 'none', fontSize: '0.9rem', transition: '0.3s' };
  ...
  const handleAddBook = async () => {
    if(!bookForm.title || !bookForm.isbn) return toast.error("Title & ISBN required"); // Removed price check to be safe, but you can keep it
  ...
  const openReturnModal = (issue) => {
      // Fine calculation logic (same as before)
  ...
  return (
    <div className="dashboard-container" style={{background: '#f8fafc', height: '100vh', display: 'flex', overflow: 'hidden'}}>
      ...
      <div className="main-content" style={{flex: 1, padding: '30px 40px', overflowY: 'auto'}}>
        ...
        <header className="slide-in-down" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '30px' }}>
          <div>
            <h1 className="gradient-text" style={{ fontSize: '2.5rem', fontWeight: '900', color: '#0f172a' }}>Library Command</h1>
            <p style={{ color: '#64748b' }}>Real-time circulation, inventory & e-library.</p>
          </div>
          <div style={{display:'flex', gap:'10px'}}>
             <div className="scanner-btn">📷 Scan</div>
        ...
        <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'20px'}}>
            ...
            {activeTab === 'books' && (
                ...
                style={{padding:'12px 20px', borderRadius:'30px', border:'1px solid #cbd5e1', width:'300px', outline:'none', background:'white', color:'#1e293b'}} /> 
            )}
        </div>

        {/* VIEW 1: BOOK CATALOG */}
        {activeTab === 'books' && (
            <div className="glass-card fade-in-up" style={{ padding: '25px', background: 'white', borderRadius: '24px' }}>
                            ...
                            <tr key={book.id} className="floating-row-glow">
                                <td>
                                    <div style={{display:'flex', alignItems:'center', gap:'10px'}}>
                                        <div style={{width:'40px', height:'50px', background:'linear-gradient(135deg, #e2e8f0, #cbd5e1)', borderRadius:'4px'}}></div>
                                        <div>
                                ...
                                <td>
                                    <div style={{fontSize:'0.85rem', fontWeight:'600', color:'#334155'}}>{book.isbn}</div>
                                    <div style={{fontSize:'0.75rem', color:'#64748b'}}>📍 {book.location || "N/A"}</div>
                                </td>
                                <td><span className="badge-gray">{book.category}</span></td>
                                {/* ✅ FIXED PRICE DISPLAY */}
                                <td><b style={{color:'#334155'}}>₹{book.price || '0'}</b></td>
                                <td>
                                    <span style={{color: book.available_copies>0?'#10b981':'#ef4444', fontWeight:'800', display:'flex', alignItems:'center', gap:'5px'}}>
                        ...
                        {issues.map(issue => {
                            ...
                            return (
                                <tr key={issue.id} className="floating-row-glow" style={{borderLeft: isLate ? '4px solid #ef4444' : 'none'}}>
                                    <td><b style={{color: '#334155'}}>{issue.student_roll}</b></td>
                                    <td><span style={{color:'#475569'}}>{issue.book_title}</span></td>
                                    <td>
                                        <span style={{color: isLate ? '#ef4444' : '#64748b', fontWeight: isLate?'bold':'normal'}}>
                                    ...
                                    <td>
                                        <span style={{background: issue.status==='Returned'?'#f0fdf4': isLate ? '#fef2f2' : '#fff7ed', color: issue.status==='Returned'?'#16a34a': isLate ? '#dc2626' : '#ea580c', padding:'4px 10px', borderRadius:'20px', fontSize:'0.75rem', fontWeight:'700'}}>
                                            {issue.status}
                                        </span>
                                    </td>
                                    <td style={{textAlign:'right'}}>
        ...
        {showDetailModal && selectedItem && (
            <div className="overlay-blur centered-flex" onClick={() => setShowDetailModal(false)}>
                <div className="modal-box slide-in-up" onClick={e=>e.stopPropagation()} style={{maxWidth:'450px'}}>
                    ...
                    <div style={{marginTop:'20px'}}>
                        ...
                        {selectedItem.pdf_url && <a href={selectedItem.pdf_url} target="_blank" className="btn-confirm-gradient" style={{display:'block', textAlign:'center', textDecoration:'none', marginTop:'20px', padding:'12px'}}>Download E-Book 📥</a>}
                    </div>
                </div>
            </div>
        )}
        ...
        {showReturnModal && selectedItem && (
            <div className="overlay-blur centered-flex" onClick={() => setShowReturnModal(false)}>
                <div className="modal-box slide-in-up" onClick={e=>e.stopPropagation()} style={{width:'450px'}}>
                    <div className="panel-header-simple">
      ...
      <style>{`
        ...
        .icon-btn { width: 34px; height: 34px; border-radius: 10px; border: none; cursor: pointer; display: inline-flex; align-items: center; justify-content: center; margin-left: 6px; transition: 0.2s; font-size: 1rem; background: #f1f5f9; color: #64748b; }
        .icon-btn:hover { transform: translateY(-2px); background: #e2e8f0; color: #0f172a; }
        
        .return-btn { background: #0f172a; border: none; color: white; padding: 6px 14px; border-radius: 8px; cursor: pointer; font-size: 0.8rem; font-weight: 700; transition: 0.2s; box-shadow: 0 4px 10px rgba(0,0,0,0.1); }
        .return-btn:hover { transform: translateY(-2px); background: #1e293b; }
        
        .badge-gray { background: #f1f5f9; padding: 5px 12px; border-radius: 8px; fontSize: 0.75rem; fontWeight: 700; color: #475569; text-transform: uppercase; }
        .pdf-badge { background: #eff6ff; color: #3b82f6; text-decoration: none; padding: 4px 10px; border-radius: 6px; font-size: 0.75rem; font-weight: 700; border: 1px solid #dbeafe; }
        
        .scanner-btn { display: flex; align-items: center; background: white; padding: 8px 15px; border-radius: 30px; border: 1px solid #cbd5e1; cursor: pointer; font-weight: 600; color: #475569; font-size: 0.9rem; transition: 0.2s; }
        .scanner-btn:hover { background: #f8fafc; border-color: #94a3b8; }

        .btn-confirm-gradient { background: linear-gradient(135deg, #0f172a 0%, #334155 100%); color: white; border-radius: 16px; font-weight: 700; cursor: pointer; border:none; box-shadow: 0 10px 20px rgba(15, 23, 42, 0.2); }
        ...
        .btn-glow { background: linear-gradient(135deg, #6366f1 0%, #a855f7 100%); border: none; color: white; padding: 12px 24px; border-radius: 50px; font-weight: 700; cursor: pointer; box-shadow: 0 8px 15px rgba(99, 102, 241, 0.25); display: flex; align-items: center; font-size: 0.9rem; }
        ...
        .icon-box-floating { width: 55px; height: 55px; border-radius: 18px; display: flex; align-items: center; justify-content: center; font-size: 1.6rem; animation: floatIcon 4s ease-in-out infinite; }
        ...
        .luxe-panel { width: 480px; height: 100%; background: white; padding: 35px; display: flex; flex-direction: column; box-shadow: -20px 0 60px rgba(0,0,0,0.15); overflow-y: auto; }
        ...
        .modal-box { background: white; padding: 30px; border-radius: 24px; box-shadow: 0 25px 80px rgba(0,0,0,0.3); width: 400px; text-align: left; }
