...
import {
  ...
  ChevronRight, LayoutGrid, CheckSquare, ArrowLeft, User, ImagePlus, Save, AlignLeft
} from "lucide-react";
...
export default function TeacherExams() {
  ...
  // Exams State (Added Draft and Time fields)
  const [exams, setExams] = useState([
    { id: 1, title: "Physics Mock Test - Term 1", subject: "Class 12 Physics", date: "2026-03-15", startTime: "10:00", endTime: "11:00", duration: "60", marks: 50, negativeMarks: "0.25", status: "upcoming" },
    { id: 2, title: "Organic Chemistry Deep Dive", subject: "Class 11 Chemistry", date: "2026-03-12", startTime: "14:00", endTime: "14:30", duration: "30", marks: 25, negativeMarks: "0", status: "active" },
    { id: 3, title: "Trigonometry Basics Quiz", subject: "Class 10 Maths", date: "2026-03-05", startTime: "11:00", endTime: "11:45", duration: "45", marks: 40, negativeMarks: "1", status: "completed" },
    { id: 4, title: "English Grammar Test", subject: "Class 10 English", date: "", startTime: "", endTime: "", duration: "45", marks: 50, negativeMarks: "0", status: "draft" },
  ]);

  // Form State (Updated with Advanced Features)
  const [testMeta, setTestMeta] = useState({
    title: "", subject: "", date: "", startTime: "", endTime: "", duration: "", marks: "", negativeMarks: "0"
  });

  // Question State (Added type and image)
  const [questions, setQuestions] = useState([
    { id: 1, type: "mcq", text: "", image: null, options: ["", "", "", ""], correctOption: 0 }
  ]);

  const dummyResults = [
    { id: 101, name: "Aarav Sharma", score: 38, max: 40, percentage: 95 },
    { id: 102, name: "Priya Patel", score: 32, max: 40, percentage: 80 },
    { id: 103, name: "Rohan Singh", score: 18, max: 40, percentage: 45 },
  ];
  ...
  // --- CRUD ACTIONS ---
  const handleSaveTest = (e, statusToSave = "upcoming") => {
    e.preventDefault();

    // Basic validation only if we are publishing. Drafts can be incomplete.
    if (statusToSave !== "draft") {
      ...
      const hasEmptyQ = questions.some(q => !q.text && !q.image);
      if (hasEmptyQ) return toast.error("Questions cannot be completely empty!");
    }

    const examData = { ...testMeta, id: editId || Date.now(), status: statusToSave };

    if (editId) {
      setExams(exams.map(ex => ex.id === editId ? examData : ex));
      toast.success(statusToSave === 'draft' ? "Draft Updated!" : "Test Updated Successfully!");
    } else {
      setExams([examData, ...exams]);
      toast.success(statusToSave === 'draft' ? "Saved to Drafts!" : "Test Published Successfully!");
    }
    resetForm();
  };
  ...
  return (
    <div className="premium-exam-wrapper">
                ...
                <div key={exam.id} className="premium-card">
                  ...
                  <div className="card-footer">
                    <button
                      ...
                      onClick={() => exam.status === 'completed' ? handleViewResults(exam) : handleEdit(exam)}
                    >
                      {exam.status === 'completed' ? 'View Results' : exam.status === 'draft' ? 'Continue Editing' : 'Manage Test'} <ChevronRight size={18} />
                    </button>
                  </div>
                </div>
              ))}
            ...
            <form> {/* Handled buttons separately via onClick */}
              ...
              <div className="builder-block">
                ...
                <div className="premium-form-grid">
                  ...
                  <div className="form-group"><label>Negative Marking (Per Wrong MCQ)</label><input type="number" step="0.25" placeholder="e.g. 0.25" value={testMeta.negativeMarks} onChange={e => setTestMeta({ ...testMeta, negativeMarks: e.target.value })} /></div>
                </div>
              </div>
              ...
              <div className="sticky-bottom-bar">
                <button type="button" className="btn-outline-large" onClick={resetForm}>Cancel</button>
                <div className="action-group">
                  <button type="button" className="btn-outline-large dark" onClick={(e) => handleSaveTest(e, "draft")}><Save size={18} /> Save as Draft</button>
                  <button type="submit" className="btn-solid-large" onClick={(e) => handleSaveTest(e, "upcoming")}>{editId && testMeta.status !== 'draft' ? "Update Published Test" : "Save & Publish Test"}</button>
                </div>
              </div>
            </form>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="results-wrapper">
            ...
            <div className="results-summary">
              <div className="summary-box">
                ...
                <h2>{dummyResults.length}</h2>
              </div>
              <div className="summary-box">
                <h4>Average Score</h4>
                <h2>{Math.round(dummyResults.reduce((a, b) => a + b.percentage, 0) / dummyResults.length)}%</h2>
              </div>
              <div className="summary-box">
                <h4>Highest Score</h4>
                <h2 className="text-green">40/40</h2>
              </div>
            </div>
              ...
              {dummyResults.map((student) => (
                <div key={student.id} className="student-result-card">
                  <div className="student-info">
                    ...
                    <div>
                      <h4>{student.name}</h4>
                      <p>ID: STD-{student.id}</p>
                    </div>
                  </div>

                  <div className="score-info">
                    <div className="score-text">
                      <strong>{student.score}</strong> / {student.max}
                    </div>
                    <div className="progress-bg">
                      <div
                        className={`progress-fill ${student.percentage < 50 ? 'fail' : 'pass'}`}
      ...
      <style>{`
        ...
        .inner-container {
            width: 100%; max-width: 1400px; margin: 0 auto; padding: 30px; 
        ...
        .premium-search-input { 
            width: 100%; padding: 18px 20px 18px 55px; border-radius: 16px; border: 2px solid #e2e8f0 !important; 
            font-size: 1.05rem; outline: none; background-color: #ffffff !important; color: #0f172a !important; 
            transition: all 0.3s; box-shadow: 0 4px 20px rgba(0,0,0,0.03);
        }
        ...
        .student-result-card { display: grid; grid-template-columns: 2fr 3fr 1fr; align-items: center; padding: 20px; border: 1px solid #f1f5f9; border-radius: 16px; margin-bottom: 15px; transition: 0.2s; background: #fdfdfd;}
        ...
        /* --- DELETE MODAL --- */
        .modal-backdrop { position: fixed; top:0; left:0; width:100%; height:100%; background: rgba(15, 23, 42, 0.6); backdrop-filter: blur(4px); display: flex; align-items: center; justify-content: center; z-index: 9999; padding: 20px;}
