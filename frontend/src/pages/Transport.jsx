...
const DUMMY_ROUTES = [
    { id: "demo1", name: "Route 1: City Center", busNumber: "UP-15-AB-1234", driver: "Ramesh Kumar", phone: "9876543210", capacity: 50, filled: 45, status: "On Time", color: "linear-gradient(135deg, #3b82f6 0%, #2563eb 100%)", startLat: 28.6139, startLng: 77.2090 },
    { id: "demo2", name: "Route 2: Shastri Nagar", busNumber: "UP-15-XY-9876", driver: "Suresh Singh", phone: "8765432109", capacity: 40, filled: 38, status: "Delayed", color: "linear-gradient(135deg, #f59e0b 0%, #d97706 100%)", startLat: 28.5355, startLng: 77.3910 },
    { id: "demo3", name: "Route 3: Delhi Road", busNumber: "UP-15-CD-4567", driver: "Mahesh Yadav", phone: "7654321098", capacity: 50, filled: 20, status: "On Time", color: "linear-gradient(135deg, #10b981 0%, #059669 100%)", startLat: 28.7041, startLng: 77.1025 },
    { id: "demo4", name: "Route 4: Kanker Khera", busNumber: "UP-15-EF-1122", driver: "Rajesh Gupta", phone: "6543210987", capacity: 30, filled: 30, status: "Full", color: "linear-gradient(135deg, #ef4444 0%, #dc2626 100%)", startLat: 28.4595, startLng: 77.0266 },
];
...
const THEME_COLORS = [
    ...
    "linear-gradient(135deg, #8b5cf6 0%, #7e22ce 100%)", // Purple
];
...
export default function Transport() {
    ...
    const [formData, setFormData] = useState({
        routeName: "", busNumber: "", driverName: "", driverPhone: "", capacity: 40, fee: 2000, vehicleType: "Bus"
    });
    ...
    // 🚀 LIVE API: Fetch Transport Data
    const fetchRoutes = async () => {
        setLoading(true);
        try {
            // Verify your endpoint in urls.py, usually it's 'transport/' 
            const res = await api.get('/transport/').catch(() => ({ data: [] }));

            if (res.data && res.data.length > 0) {
                // Map DB fields to Frontend UI fields
                const mappedData = res.data.map((item, idx) => ({
                    id: item.id,
                    name: item.route_name,
                    busNumber: item.vehicle_number,
                    driver: item.driver_name,
                    phone: item.driver_phone,
                    capacity: item.capacity,
                    vehicleType: item.vehicle_type,
                    // Generated UI Elements
                    filled: Math.floor(item.capacity * 0.8), // Placeholder logic for occupancy
                    status: "On Time", // Default status
                    color: THEME_COLORS[idx % THEME_COLORS.length],
                    startLat: 28.6139 + (idx * 0.02), // Spread buses on map
                    startLng: 77.2090 + (idx * 0.02)
                }));
                setRoutes(mappedData);
            } else {
                setRoutes(DUMMY_ROUTES); // 🟢 Auto-fallback
            }
        } catch (err) {
            console.error(err);
    ...
    const handleOpenAdd = () => {
        ...
        setFormData({ routeName: "", busNumber: "", driverName: "", driverPhone: "", capacity: 40, fee: 2000, vehicleType: "Bus" });
        setActivePanel("form");
    };
    ...
    const handleEdit = (route) => {
        ...
        setFormData({
            ...
            driverPhone: route.phone, capacity: route.capacity, fee: 2000, vehicleType: route.vehicleType || "Bus"
        });
        setActivePanel("form");
    };
    ...
    // 🚀 LIVE API: Delete Route
    const handleDelete = async () => {
        const loadToast = toast.loading("Deleting route...");
        try {
            if (!selectedRoute.id.toString().includes("demo")) {
                await api.delete(`/transport/${selectedRoute.id}/`);
            }
            toast.success("Route Deleted Successfully! 🗑️", { id: loadToast });
            fetchRoutes();
        } catch (err) {
            toast.error("Failed to delete route", { id: loadToast });
        } finally {
            setActivePanel("none");
        }
    };

    // 🚀 LIVE API: Save/Update Route
    const handleSave = async () => {
        if (!formData.routeName || !formData.busNumber) {
            toast.error("Please fill required fields!");
            return;
        }

        const loadToast = toast.loading(selectedRoute ? "Updating Route..." : "Adding Route...");

        // Map UI form to DB model format
        const payload = {
            route_name: formData.routeName,
            vehicle_number: formData.busNumber,
        ...
        try {
            if (selectedRoute && !selectedRoute.id.toString().includes("demo")) {
                // Update
                await api.patch(`/transport/${selectedRoute.id}/`, payload);
                toast.success("Route Updated Successfully! ✅", { id: loadToast });
            } else if (!selectedRoute) {
                // Create
                await api.post(`/transport/`, payload);
                toast.success("New Vehicle Route Added! 🚌", { id: loadToast });
            } else {
                toast.success("Demo Route Updated in UI! ✅", { id: loadToast });
            }

            fetchRoutes(); // Refresh from DB
            setActivePanel("none");
        } catch (err) {
                                ...
                                {activePanel === 'form' && (
                                    <>
                                        ...
                                        <div className="grid-2-col">
                                            <div className="input-group"><label>Bus/Van Number</label><input type="text" placeholder="e.g. UP-15-AB-9999" value={formData.busNumber} onChange={(e) => setFormData({ ...formData, busNumber: e.target.value })} style={inputStyle} /></div>
