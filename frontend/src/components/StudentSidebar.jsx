...
export default function StudentSidebar() {
    ...
    const NavButton = ({ icon: Icon, label, path }) => {
        ...
        return (
            <motion.button
                onClick={() => {
                    navigate(path);
                    setIsOpen(false); // Mobile pe click karte hi turant close ho jaye
                }}
                whileHover={{ x: 8, backgroundColor: active ? 'var(--primary)' : '#f8fafc', scale: 1.02 }}
                whileTap={{ scale: 0.95 }}
                style={{
                    display: 'flex', alignItems: 'center', gap: '15px', background: active ? 'var(--primary)' : 'transparent',
                    border: 'none', padding: '14px 18px', borderRadius: '16px', fontSize: '1.05rem', fontWeight: '700',
                    color: active ? 'white' : 'var(--text-muted)', cursor: 'pointer', transition: 'all 0.3s ease', width: '100%', textAlign: 'left',
    ...
    return (
        <>
            {/* 📱 MOBILE TOP HEADER (Fixed to top on mobile only) */}
            ...
            {/* 📱 MOBILE SIDEBAR OVERLAY (Dim background when open) */}
            <AnimatePresence>
                {isOpen && (
                    <motion.div
                ...
                <div className="sidebar-brand">
                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                        <motion.div className="brand-icon" animate={{ boxShadow: ["0px 0px 0px rgba(79,70,229,0)", "0px 0px 15px rgba(79,70,229,0.6)", "0px 0px 0px rgba(79,70,229,0)"] }} transition={{ duration: 2, repeat: Infinity }}>
                            <BookOpen size={24} color="#ffffff" style={{ background: 'var(--primary)', padding: '6px', borderRadius: '10px', width: '36px', height: '36px' }} />
                        </motion.div>
                        <h2 style={{ fontSize: '1.4rem', fontWeight: '900', color: '#1e293b', margin: 0 }}>SHIV ADDA</h2>
                    </div>
                    {/* Close button for mobile explicitly shown */}
                    <button className="close-sidebar-btn" onClick={() => setIsOpen(false)}>
                        <X size={26} color="#ef4444" />
                    </button>
                </div>

                {/* YAHAN SE STAGGER ANIMATION HATA DIYA GAYA HAI TAQI LATE SHOW NA HO */}
            ...
            {/* 🚀 ULTIMATE RESPONSIVE COMPONENT SCOPED CSS */}
            <style jsx="true">{`
                ...
                /* Desktop pe inko hide rakhna hai */
                ...
                @media (max-width: 1024px) {
                    /* Sidebar ko screen ke bahar feko */
                    .glass-sidebar { 
                        transform: translateX(-100%); 
                        z-index: 1001; /* Overlay ke upar aana chahiye */
                        box-shadow: none;
                    }
                    /* Jab open ho toh andar lao */
                    .glass-sidebar.mobile-open { 
                        transform: translateX(0); 
                        box-shadow: 20px 0 50px rgba(0,0,0,0.3); 
                    }
                    
                    /* Mobile wale elements show karo */
                    ...
                    /* Overlay Sidebar ke peeche, par baki page ke aage */
                    .mobile-overlay { 
                        display: block; position: fixed; inset: 0; background: rgba(15,23,42,0.6);
                        z-index: 995; 
                    }
                }

                @media (max-width: 480px) {
                    .glass-sidebar { width: 260px; padding: 20px; }
                }
            `}</style>
        </>
    );
