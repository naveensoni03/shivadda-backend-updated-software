...
print(f"ðŸ”§ Connecting Filters to Table Data in: {TARGET_FILE}")
...
export default function Locations() {
  ...
  // --- STATIC CONTINENT MAPPING (Since DB doesn't have continent column) ---
  const continentMap = {
      "India": "Asia",
      "China": "Asia",
      "Japan": "Asia",
  ...
  // Filter the Table Data based on Dropdowns
  const filteredCenters = centers.filter(center => {
      // 1. Continent Filter
      if (selectedContinent) {
          // Find country of this center by matching address city
          const centerCity = locationsDB.find(l => center.address.includes(l.city));
          if (centerCity && continentMap[centerCity.country] !== selectedContinent) return false;
      }

      // 2. Country Filter
      if (selectedCountry) {
          // Check if address contains a city that belongs to selectedCountry
          const citiesInCountry = locationsDB.filter(l => l.country === selectedCountry).map(l => l.city);
          const hasCity = citiesInCountry.some(city => center.address.includes(city));
          if (!hasCity && !center.address.includes(selectedCountry)) return false; 
      }

      // 3. State Filter
      if (selectedState) {
          const citiesInState = locationsDB.filter(l => l.state === selectedState).map(l => l.city);
          const hasCity = citiesInState.some(city => center.address.includes(city));
          if (!hasCity && !center.address.includes(selectedState)) return false;
      }
  ...
  const saveEdit = () => {
      toast.success("Center Updated Successfully!", { style: { background: '#10b981', color: '#fff' } });
      setEditNode(null);
  };

  const handleAddSubmit = () => {
      if(!newCenterData.name) return toast.error("Center Name is required!");
      const newEntry = {
          id: centers.length + 100,
      ...
      toast.success("New Node Onboarded!", { icon: 'ðŸš€', style: { background: '#3b82f6', color: '#fff' } });
  };
  ...
  return (
    <div className="waoo-app-container">
                            ...
                            {currentItems.length > 0 ? currentItems.map((center, i) => (
                                <tr key={center.id} className="waoo-row">
                                    ...
                                    <td style={{textAlign: 'center', position: 'relative'}}>
                                        <button className="row-action-btn" onClick={(e) => { e.stopPropagation(); setActiveActionMenu(activeActionMenu === center.id ? null : center.id); }}>
                                        ...
                                        {activeActionMenu === center.id && (
                                            <div className="action-popup fade-in">
                                                ...
                                                <button onClick={(e) => { e.stopPropagation(); toast.error("Delete: " + center.name); setActiveActionMenu(null); }} className="action-item delete"><Trash2 size={14}/> Remove</button>
                                            </div>
                                        )}
                                    </td>
                                </tr>
                            )) : (
                                <tr><td colSpan="6" style={{textAlign:'center', padding:'30px', color:'#64748b'}}>No Nodes found in this Sector.</td></tr>
                            )}
                        </tbody>
                    </table>
                </div>
                    ...
                    <div className="modal-content scale-up-bounce">
                        ...
                        <div className="modal-body">
                            <div className="hero-name-card">
                                <h3>{viewNode.name}</h3>
                                <span className="id-tag">ID: {viewNode.id}08-NODE</span>
                            </div>
                            <div className="detail-grid">
                                <div className="info-item">
                                    ...
                                    <div>
                                        ...
                                        <p>{viewNode.address || "Grid Coordinate Alpha-7"}</p>
                                    </div>
                                </div>
                                <div className="info-item">
                                    ...
                                    <div>
                                        ...
                                        <p>{viewNode.contact_email}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div className="modal-footer">
                            <button className="btn-close" onClick={() => setViewNode(null)}>Close</button>
                        </div>
                    </div>
                </div>
            )}
                        ...
                        <div className="modal-body">
                            <div className="edit-form-grid">
                                <div className="form-group">
                                    ...
                                    <input type="text" defaultValue={editNode.name} className="modal-input" />
                                </div>
                                <div className="form-group">
                                    <label>Location / Address</label>
                                    <input type="text" defaultValue={editNode.address} className="modal-input" />
                                </div>
                                <div className="form-group">
                                    <label>Email Contact</label>
                                    <input type="email" defaultValue={editNode.contact_email} className="modal-input" />
                                </div>
                            </div>
                        </div>
                        <div className="modal-footer">
                            <button className="btn-cancel" onClick={() => setEditNode(null)}>Cancel</button>
                            <button className="btn-save" onClick={saveEdit}><Save size={18}/> Save Changes</button>
                        </div>
                    </div>
                </div>
            )}
                ...
                <div className="modal-overlay fade-in">
                    <div className="modal-content scale-up-bounce">
                        ...
                        <div className="modal-body">
                            <div className="edit-form-grid">
                                <div className="form-group">
                                    <label>Center Identity</label>
                                    <input 
                                        type="text" 
                                        placeholder="e.g. Shivadda North Campus" 
                                ...
                                <div className="form-group">
                                    ...
                                    <input 
                                        ...
                                        onChange={(e) => setNewCenterData({...newCenterData, email: e.target.value})}
                                    />
                                </div>
                            </div>
                        </div>
                        <div className="modal-footer">
                            ...
                            <button className="btn-save" style={{background: '#3b82f6'}} onClick={handleAddSubmit}>
                                <CheckCircle size={18}/> Initialize
                            </button>
                        </div>
                    </div>
                </div>
        ...
        <style>{`
            ...
            .hero-sub { color: #334155; font-size: 1.1rem; font-weight: 500; }
            .deck-header { font-weight: 900; color: #0f172a; font-size: 0.8rem; letter-spacing: 1px; margin-bottom: 15px; }
            .waoo-input-box label { font-size: 0.75rem; font-weight: 800; color: #334155; margin-bottom: 5px; }
            .premium-select { background: #ffffff !important; color: #0f172a !important; font-weight: 700; border: 2px solid #e2e8f0; }
            ...
            .waoo-input-box { flex: 1; display: flex; flex-direction: column; }
            ...
            .waoo-list-section { background: white; border-radius: 32px; border: 1px solid #e2e8f0; position: relative; }
            .list-top { padding: 30px; display: flex; justify-content: space-between; align-items: center; }
            .waoo-primary-btn { background: #3b82f6; color: white; border: none; padding: 16px 32px; border-radius: 18px; font-weight: 800; cursor: pointer; display: flex; gap: 10px; align-items: center; }
            
            .waoo-main-table { width: 100%; border-collapse: collapse; }
            .waoo-main-table th { text-align: left; padding: 20px 30px; color: #475569; font-size: 0.75rem; font-weight: 800; text-transform: uppercase; }
            .waoo-row { border-bottom: 1px solid #f8fafc; }
            .waoo-row:hover { background: #f8fafc; }
            .row-name { font-weight: 800; color: #0f172a; }
            .row-addr { color: #334155; font-weight: 600; }
            .row-mail { color: #0f172a; font-weight: 600; }
            .row-id { padding-left: 30px; color: #64748b; font-weight: 700; }
            ...
            .waoo-pagination-bar { padding: 20px 30px; border-top: 1px solid #f1f5f9; background: #fcfcfc; display: block !important; }
            .p-content { display: flex; justify-content: space-between; align-items: center; }
            .p-nav { background: white; border: 1px solid #e2e8f0; padding: 8px 16px; border-radius: 10px; cursor: pointer; display: flex; align-items: center; gap: 8px; font-weight: 700; color: #0f172a; transition: 0.2s; }
            .p-nav:hover:not(:disabled) { background: #3b82f6; color: white; border-color: #3b82f6; }
            .p-nav:disabled { opacity: 0.4; cursor: not-allowed; }
            ...
            .modal-content { background: white; width: 500px; border-radius: 24px; box-shadow: 0 25px 50px -12px rgba(0,0,0,0.5); overflow: hidden; }
            .modal-header { padding: 25px; border-bottom: 1px solid #f1f5f9; display: flex; justify-content: space-between; align-items: center; }
            .modal-title-box { display: flex; align-items: center; gap: 12px; }
            .modal-title-box h2 { margin: 0; font-size: 1.25rem; font-weight: 800; color: #0f172a; }
            .close-x { background: #f1f5f9; border: none; width: 40px; height: 40px; border-radius: 12px; cursor: pointer; display: flex; align-items: center; justify-content: center; color: #64748b; transition: 0.2s; }
            .close-x:hover { background: #fee2e2; color: #ef4444; transform: rotate(90deg); }
            
            .modal-body { padding: 30px; }
            .hero-name-card { background: #f8fafc; padding: 20px; border-radius: 16px; border: 1px solid #e2e8f0; margin-bottom: 25px; }
            .hero-name-card h3 { margin: 0; font-size: 1.5rem; font-weight: 900; color: #0f172a; }
            .id-tag { font-size: 0.75rem; font-weight: 800; color: #3b82f6; text-transform: uppercase; letter-spacing: 1px; }
            
            .detail-grid { display: flex; flex-direction: column; gap: 20px; }
            .info-item { display: flex; gap: 15px; align-items: flex-start; }
            .info-item label { display: block; font-size: 0.75rem; font-weight: 800; color: #64748b; text-transform: uppercase; margin-bottom: 4px; }
            .info-item p { margin: 0; font-weight: 700; color: #1e293b; line-height: 1.4; }

            .modal-footer { padding: 20px 30px; background: #fcfcfc; text-align: right; border-top: 1px solid #f1f5f9; display: flex; gap: 10px; justify-content: flex-end; }
            .btn-close { background: #0f172a; color: white; border: none; padding: 12px 24px; border-radius: 12px; font-weight: 800; cursor: pointer; transition: 0.2s; }
            .btn-close:hover { background: #1e293b; transform: translateY(-2px); }

            .edit-form-grid { display: flex; flex-direction: column; gap: 20px; }
            .form-group { display: flex; flex-direction: column; gap: 8px; }
            .form-group label { font-size: 0.8rem; font-weight: 800; color: #334155; }
            .modal-input { 
                padding: 12px; border: 2px solid #e2e8f0; border-radius: 12px; 
                font-weight: 600; color: #0f172a; background: #f8fafc; font-size: 1rem; 
            }
            .modal-input:focus { border-color: #3b82f6; background: white; outline: none; }
            
            .btn-save { background: #10b981; color: white; border: none; padding: 12px 24px; border-radius: 12px; font-weight: 800; cursor: pointer; display: flex; align-items: center; gap: 8px; }
            .btn-save:hover { background: #059669; }
            .btn-cancel { background: white; border: 1px solid #e2e8f0; color: #64748b; padding: 12px 20px; border-radius: 12px; font-weight: 800; cursor: pointer; }
            .btn-cancel:hover { background: #f1f5f9; color: #0f172a; }

            .fade-in { animation: fadeIn 0.3s ease; }
            @keyframes fadeIn { from { opacity: 0; } to { opacity: 1; } }
            .scale-up-bounce { animation: scaleUp 0.4s cubic-bezier(0.175, 0.885, 0.32, 1.275); }
...
print("✅ FILTERS & UI FIXED: Dropdowns now actively filter table data!")
