...
print(f"✨ APPLYING 'WAAOO' UI REDESIGN TO: {TARGET_FILE}")

code_content = r"""import React, { useState, useEffect } from "react";
import SidebarModern from "../components/SidebarModern";
import { Globe, Plus, RefreshCw, MoreHorizontal, ChevronLeft, ChevronRight, Zap, Eye, Edit, Trash2, X, MapPin, Mail, Server, Save, CheckCircle, Search, Filter } from "lucide-react";
...
export default function Locations() {
  ...
  const handleAddSubmit = () => {
      ...
      toast.success("New Node Added!", { icon: '🚀' });
  };
  ...
  return (
    <div className="waoo-app-container">
                        ...
                        {viewNode && (
                            <div className="g-modal-body">
                                <div className="intel-card">
                                    <div className="intel-row"><span className="lbl">Identity</span> <span className="val">{viewNode.name}</span></div>
                                    <div className="intel-row"><span className="lbl">Location</span> <span className="val">{viewNode.address}</span></div>
                                    <div className="intel-row"><span className="lbl">Uplink</span> <span className="val highlight">{viewNode.contact_email}</span></div>
                                </div>
                            </div>
                        )}
                        ...
                        {(editNode || isAddOpen) && (
                            <div className="g-modal-body">
                                <div className="waoo-form">
                                    ...
                                    />
                                    <label>Grid Coordinates (Address)</label>
                                    <input 
                                        className="waoo-input" 
                                ...
                                <div className="g-modal-foot">
                                    <button className="waoo-btn-primary" onClick={editNode ? () => toast.success("Updated") : handleAddSubmit}>
        ...
        <style>{`
            ...
            .waoo-select { border: none; background: transparent; font-weight: 700; color: #475569; font-size: 0.95rem; cursor: pointer; outline: none; padding: 8px; border-radius: 8px; transition: 0.2s; }
            ...
            .glass-modal { background: rgba(255, 255, 255, 0.95); border: 1px solid white; padding: 30px; border-radius: 30px; width: 450px; box-shadow: 0 25px 50px -12px rgba(0,0,0,0.3); }
            ...
            .waoo-btn-primary { width: 100%; padding: 14px; background: linear-gradient(135deg, #6366f1 0%, #8b5cf6 100%); color: white; border: none; border-radius: 14px; font-weight: 800; font-size: 1rem; cursor: pointer; box-shadow: 0 10px 20px -5px rgba(99, 102, 241, 0.4); transition: 0.2s; }
            .waoo-btn-primary:hover { transform: translateY(-2px); filter: brightness(1.1); }

            /* ANIMATIONS */
            .fade-in-down { animation: fadeInDown 0.6s ease; }
            .fade-in-up { animation: fadeInUp 0.6s ease; }
            .fade-in-up-delay { animation: fadeInUp 0.8s ease; }
...
print("✨ WAAOO UI APPLIED: Glassmorphism, Floating Cards & Ultra-Modern Look!")
