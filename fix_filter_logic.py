...
print(f"ðŸ”§ REPAIRING FILTER LOGIC (SMART CITY MATCHING) in: {TARGET_FILE}")
...
export default function Locations() {
  ...
  // ROBUST DATA FOR MAPPING CITIES TO STATES
  ...
  const fetchGlobalData = async () => {
      ...
      try {
          ...
          // If API returns empty locations, use fallback so filters still work
          setLocationsDB(locRes.data.length > 0 ? locRes.data : fallbackLocations);
      } catch (error) {
          console.error("Connection Error", error);
          setLocationsDB(fallbackLocations);
      } finally {
          setLoading(false);
      }
  };
  ...
  const getFilteredStates = () => {
      // Get states belonging to the selected country
      return [...new Set(locationsDB
  ...
  const filteredCenters = centers.filter(center => {
      ...
      const countrySel = selectedCountry;
      const stateSel = selectedState;
      ...
      if (selectedContinent) {
          ...
          // Also check if the city implies the continent
      ...
      // 2. Country Filter
      if (selectedCountry) {
          // Check explicit country name
          const hasCountryName = addr.includes(countrySel.toLowerCase());
          
          // SMART CHECK: Check if address contains ANY city belonging to this country
          const citiesInCountry = locationsDB
              .filter(l => l.country === countrySel)
              .map(l => l.city.toLowerCase());
          
          const hasCityMatch = citiesInCountry.some(city => addr.includes(city));

          if (!hasCountryName && !hasCityMatch) return false;
      }

      // 3. State Filter
      if (selectedState) {
          // Check explicit state name
          const hasStateName = addr.includes(stateSel.toLowerCase());

          // SMART CHECK: Check if address contains ANY city belonging to this State
          // (e.g. If State is Karnataka, does address have "Bangalore"?)
          const citiesInState = locationsDB
              .filter(l => l.state === stateSel)
              .map(l => l.city.toLowerCase());

          const hasCityMatch = citiesInState.some(city => addr.includes(city));

          if (!hasStateName && !hasCityMatch) return false;
      }
  ...
  return (
    <div className="waoo-app-container">
                        ...
                        {viewNode && (
                            <div className="g-modal-body">
                                <div className="intel-card">
                                    <div className="intel-row"><span className="lbl">Identity</span> <span className="val highlight">{viewNode.name}</span></div>
                                    <div className="intel-row"><span className="lbl">Location</span> <span className="val">{viewNode.address || "N/A"}</span></div>
                                    <div className="intel-row"><span className="lbl">Uplink</span> <span className="val">{viewNode.contact_email || "N/A"}</span></div>
                                </div>
                            ...
                            <div className="g-modal-body">
                                <div className="waoo-form">
                                    <div className="waoo-form-group">
                                        ...
                                        <input className="waoo-input-modern" placeholder="e.g. Alpha Base" defaultValue={editNode ? editNode.name : newCenterData.name} onChange={(e) => editNode ? null : setNewCenterData({...newCenterData, name:e.target.value})}/>
                                    </div>
                                    <div className="waoo-form-group">
                                        <label className="waoo-label">Grid Coordinates (Address)</label>
                                        <input className="waoo-input-modern" placeholder="e.g. Sector 7, NY" defaultValue={editNode ? editNode.address : newCenterData.address} onChange={(e) => editNode ? null : setNewCenterData({...newCenterData, address:e.target.value})}/>
                                    </div>
                                    <div className="waoo-form-group">
                                        <label className="waoo-label">Secure Comms (Email)</label>
                                        <input className="waoo-input-modern" placeholder="admin@base.com" defaultValue={editNode ? editNode.contact_email : newCenterData.email} onChange={(e) => editNode ? null : setNewCenterData({...newCenterData, email:e.target.value})}/>
                                    </div>
                                </div>
                                <div className="g-modal-foot">
        ...
        <style>{`
            :root { --primary: #6366f1; --primary-glow: rgba(99, 102, 241, 0.4); --bg-body: #f3f4f6; --text-dark: #1e293b; --text-light: #64748b; }
            ...
            .waoo-input-modern { width: 100%; padding: 14px; background: #f8fafc; border: 2px solid #e2e8f0; border-radius: 14px; font-weight: 600; color: #1e293b; font-size: 1rem; transition: all 0.25s ease; }
...
print("✅ FILTERS FIXED: Now matches Cities & States correctly!")
