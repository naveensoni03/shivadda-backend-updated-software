...
print(f"ðŸ”§ Applying ROBUST Filter Logic (Case Insensitive) in: {TARGET_FILE}")
...
export default function Locations() {
  ...
  const filteredCenters = centers.filter(center => {
      ...
      if (selectedContinent) {
          // Check if address matches any country in the continent
          ...
          // Check if address matches any CITY in the continent
          const citiesInCont = locationsDB.filter(l => continentMap[l.country] === selectedContinent).map(l => l.city.toLowerCase());
          const hasCityMatch = citiesInCont.some(city => addr.includes(city));

          if (!hasCountryMatch && !hasCityMatch) return false;
      }

      // 2. Country Filter
      if (selectedCountry) {
          // Does address contain country name? (e.g. "India")
          const directMatch = addr.includes(countrySel);
          
          // Does address contain any city of that country? (e.g. "Mumbai")
          const citiesInCountry = locationsDB.filter(l => l.country === selectedCountry).map(l => l.city.toLowerCase());
          const cityMatch = citiesInCountry.some(city => addr.includes(city));

          if (!directMatch && !cityMatch) return false;
      }

      // 3. State Filter
      if (selectedState) {
          const directMatch = addr.includes(stateSel);
          const citiesInState = locationsDB.filter(l => l.state === selectedState).map(l => l.city.toLowerCase());
          const cityMatch = citiesInState.some(city => addr.includes(city));
  ...
  const saveEdit = () => {
      toast.success("Center Updated!", { style: { background: '#10b981', color: '#fff' } });
      setEditNode(null);
  };
  ...
  const handleAddSubmit = () => {
      ...
      toast.success("New Node Added!", { icon: 'ðŸš€', style: { background: '#3b82f6', color: '#fff' } });
  };
...
print("✅ FILTERS & UI FIXED: Robust logic applied.")
